...
var _Sources = (() => {
  ...
  var manga3asq = class {
    ...
    arabicDate(value) {
      ...
      const month = match ? months[match[2] ?? ""] : void 0;
      if (!match || month === void 0) {
        return void 0;
      }
    ...
    listingUrl(page, order) {
      return page <= 1 ? `${DOMAIN}/manga/?m_orderby=${order}` : `${DOMAIN}/manga/page/${page}/?m_orderby=${order}`;
    }
    async getSearchTags() {
    ...
    async getSearchResults(query, metadata) {
      ...
      const url = page <= 1 ? `${DOMAIN}/?${parts.join("&")}` : `${DOMAIN}/page/${page}/?${parts.join("&")}`;
      const results = this.searchRows(await this.fetch(url));
      return App.createPagedResults({
        results,
        metadata: results.length === 0 ? void 0 : { page: page + 1 }
      });
    }
    async getMangaDetails(mangaId) {
      const $ = this.cheerio.load(await this.fetch(this.getMangaShareUrl(mangaId)));
      const title = this.decode(
        $("div.post-title h1").first().children().remove().end().text() || ($('meta[property="og:title"]').attr("content") ?? "")
      );
      const desc = this.decode(
        $("div.description-summary div.summary__content, div.summary__content, div.manga-excerpt").first().text()
      );
      const genres = [];
      const seen = /* @__PURE__ */ new Set();
      ...
      return App.createSourceManga({
        ...
        mangaInfo: App.createMangaInfo({
          ...
          ...genres.length > 0 ? { tags: [App.createTagSection({ id: "genres", label: "Genres", tags: genres })] } : {}
        })
      });
    }
